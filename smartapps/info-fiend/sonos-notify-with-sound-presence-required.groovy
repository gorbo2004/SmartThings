 ...
 */
definition(
	name: "Sonos Notify with Sound - Presence Required",
	namespace: "info_fiend",
	author: "SmartThings + anthony pastor",
	description: "Play a sound or custom message through your Sonos when the mode changes or other events occur.  Presence is required -- NOT a trigger.",
	category: "My Apps",
...
def mainPage() {
	dynamicPage(name: "mainPage") {
        ...
        section ("Identify presence sensors:"){
        	input "myPresence", "capability.presenceSensor", title: "Selected sensor must be present.", required: true, multiple: false
        }    
...
def subscribeToEvents() {
	...
	subscribe(myPresence, "presence.present", presenceHandler)
	subscribe(myPresence, "presence.not present", presenceHandler)    
...
def eventHandler(evt) {
	...
	if (state.someoneHome) {
    	log.info "Eventhandler: Someone home."
    	...
    	log.info "Eventhandler: No one home."
    }    
}
...
def presenceHandler(evt) { 
	...
	if (evt.value == "present") {
   		log.info "presenceHandler: state.someoneHome is now TRUE." 
        state.someoneHome = true
    } else {
   		log.info "presenceHandler: state.someoneHome is now FALSE." 
        state.someoneHome = false
    }    
}
...
def checkPresence() {
   	...
   	if (myPresence.currentValue("presence") == "present") {
   		log.info "checkPresence: state.someoneHome is now TRUE." 
        state.someoneHome = true
    } else {
   		log.info "checkPresence: state.someoneHome is now FALSE." 
        state.someoneHome = false
    } 
