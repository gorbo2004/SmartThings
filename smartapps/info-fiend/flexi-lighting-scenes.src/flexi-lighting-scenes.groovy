        ...
        theModes?.each() {
        	    ...
        	    section("${name} Scene:", hideable:true, hidden: state.installed) {
	                ...
	                settings.hueMaster?.each() {
                	    ...
                	    input "${it.displayName}_${name}_HMtrig", "enum", title: "Should a MODE change cause lights to turn on?",
    	            ...
    	            settings.hueFrees?.each() {
						...
						["Warm", "Soft", "Normal", "Daylight", "Red", "Green", "Blue", "Yellow", "Orange", "Purple", "Pink"]], defaultValue: "Warm"
                    	input "${it.displayName}_${name}_HFtrig", "enum", title: "Should a MODE change cause lights to turn on?",
                        	metadata:[values: ["yes", "no"]], required:true, defaultValue: "no"
	                }
    	            settings.dimFrees?.each() {
            	        ...
            	        input "${it.displayName}_${name}_Dtrig", "enum", title: "Should a MODE change cause lights to turn on?",
                	        metadata:[values: ["yes", "no"]], required:true, defaultValue: "no"                    
	                }
